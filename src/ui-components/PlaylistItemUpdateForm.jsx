...
export default function PlaylistItemUpdateForm(props) {
  const {
    id,
    playlistItem,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = { ...initialValues, ...playlistItemRecord };
    setPlaylistName(cleanValues.playlistName);
    setVideoTitle(cleanValues.videoTitle);
    setVideoDescription(cleanValues.videoDescription);
    setVideoPlays(cleanValues.videoPlays);
    setVideoThumbnail(cleanValues.videoThumbnail);
    setErrors({});
  };
  const [playlistItemRecord, setPlaylistItemRecord] =
    React.useState(playlistItem);
  React.useEffect(() => {
    const queryData = async () => {
      const record = id
        ? await DataStore.query(PlaylistItem, id)
        : playlistItem;
      setPlaylistItemRecord(record);
    };
    queryData();
  }, [id, playlistItem]);
  React.useEffect(resetStateValues, [playlistItemRecord]);
  const validations = {
    playlistName: [],
    videoTitle: [],
    videoDescription: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          await DataStore.save(
            PlaylistItem.copyOf(playlistItemRecord, (updated) => {
      ...
      {...rest}
      {...getOverrideProps(overrides, "PlaylistItemUpdateForm")}
    >
      <TextField
        label="Playlist name"
        isRequired={false}
